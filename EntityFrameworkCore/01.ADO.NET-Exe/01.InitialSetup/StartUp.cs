namespace _01.InitialSetup
{
    ...
    public class StartUp
    {
        public static void Main(string[] args)
        {
            ...
            {
                //Create tables
                var createTables = GetCreateTables();

                foreach (var table in createTables)
                {
                    ExecuteNonQuery(sqlConnection, table);
                }
            
                //Insert data in tables
                var insertDataInTables = GetInsertDataInTables();

                foreach (var cmd in insertDataInTables)
                {
        ...
        private static string[] GetInsertDataInTables()
        {
